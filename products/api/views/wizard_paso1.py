...
def validate_ramo_path(path_ids, modalidades):
    """
    Reusa la misma lógica del catálogo (llamando al selector SQL directamente)
    """
    # Cargamos ítems:
    items = []
    with connection.cursor() as cur:
        for pid in path_ids:
            ...
            items.append({
                "id": row[0], "type": row[1], "code": row[2], "name": row[3],
                "enabled": row[4], "parent_id": row[5], "level": row[6], "meta": row[7],
            })

    # N3 mínimo:
    if (items[-1]["level"] or 0) < 3:
        return False, "Debes llegar al menos al Nivel 3 (Ramo Actuarial)."

    # Requiere N5 para ciertos paths:
    ...
    if requires_level5(items) and (items[-1]["level"] or 0) < 5:
    ...
    # Modalidades cuando el leaf lo pida
    ...
    leaf = items[-1]
    if is_multi(leaf["meta"], leaf["code"], leaf["name"]):
...
class WizardCreateDraftView(APIView):
    ...
    def post(self, request):
        ...
        if not company_id or not nombre_comercial or not nombre_poliza:
            return Response({"detail": "company_id, nombre_comercial y nombre_poliza son requeridos."}, status=400)
        ...
        # TODO: inserciones reales en tus tablas de dominio
        # For demo, devolvemos ids simulados. Conecta aquí con tus repos/use_cases.
        # Ejemplo minimalista: insertar “producto_draft” y “version_draft” con ramos asociados
        with connection.cursor() as cur:
            cur.execute("SELECT gen_random_uuid()")  # postgres pgcrypto
            ...
            # Persistirías además: company_id, nombres, is_combinado y ramos/path/modalidades en tablas auxiliares.
