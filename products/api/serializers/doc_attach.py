    ...
    if section == "CG":
        ...
        """,
            [
                ...
                item["version"],
                item["idramo"],
                bool(item.get("genera_prima", False)),
            ],
        )
    elif section == "ANNEX":
        cur.execute(
...
def _insert_link(
    cur, section: str, vp_id: str, specific_id: str, link: Dict[str, Any]
) -> str:
...
def attach_document_to_version(
    ...
    with connection.cursor() as cur:
        ...
        _insert_receipt(cur, case_id, section, doc_id, link_table, link_id, file_meta)

    return AttachResult(
        link_id=link_id, document_id=doc_id, specific_id=specific_id, case_id=case_id
    )
