...
def create_ra_and_links(
    ...
    with connection.cursor() as cur:
        ...
        cur.execute("""
            ...
            RETURNING id
        """, [
            ra["idmoneda"], ra["idtabla_mortalidad"], ra["idtipo_estudio"],
            ra["ga"], ra["it"], ra["utilidad_lim"], ra["tarifa_inmediata"],
            ra.get("vigencia_desde"), ra.get("vigencia_hasta")
        ])
        ra_id = cur.fetchone()[0]

        # Enlaces RA ↔ CG/CP
        vrange = f"[{ra['vigencia_desde']},{ra['vigencia_hasta']}]" if (ra.get("vigencia_desde") and ra.get("vigencia_hasta")) else \
            (f"[{ra['vigencia_desde']},)" if ra.get(
                "vigencia_desde") else "(,)")
        for cg_id in enlaces.get("cg_ids", []):
            ...
            """, [ra_id, cg_id, vrange])  # NOTE: si usas otra tabla para RA↔CG cambia aquí
