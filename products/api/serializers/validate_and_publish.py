...
def validate_version(version_id: str) -> ValidateResult:
    with connection.cursor() as cur:
        try:
            cur.execute("""SELECT core.fn_validate_before_publish(%s)""", [version_id])
...
@transaction.atomic
def publish_version(
    version_id: str, vigencia_desde: str, vigencia_hasta: Optional[str]
) -> PublishResult:
    with connection.cursor() as cur:
        if vigencia_hasta:
            cur.execute(
                """SELECT core.fn_publicar_version(%s, %s::date, %s::date)""",
            ...
            cur.execute(
                """SELECT core.fn_publicar_version(%s, %s::date, NULL)""",
