...
def get_wizard_status(version_id: str) -> WizardStatus:
    with connection.cursor() as cur:
        cur.execute(
            "SELECT idproduct, estado, locked_by_state FROM core.version_product WHERE id=%s",
            [version_id],
        )
        row = cur.fetchone()
        if not row:
        ...
        cur.execute(
            "SELECT count(*) FROM link.vp_to_annex WHERE idversionproduct=%s",
            [version_id],
        )
        annex_count = cur.fetchone()[0]

        cur.execute(
            "SELECT count(*) FROM link.vp_to_format WHERE idversionproduct=%s",
            [version_id],
        )
